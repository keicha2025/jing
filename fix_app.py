...
const fetchTrip = async () => {
  try {
    // 🔥 這邊是關鍵：如果這行無法連線，請嘗試換成 Codespaces 的公開網址
    ...
    console.error(err)
    error.value = "無法連接後端。請確認後端 (Port 8000) 是否正在執行。"
    loading.value = false
  }
}
...
# 使用絕對路徑或相對根目錄的路徑
file_path = "travel-planner/frontend/src/App.vue"

try:
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(vue_content)
    print(f"✅ 成功！已寫入 {file_path}")
except Exception as e:
    print(f"❌ 寫入失敗: {e}")
    # 嘗試檢查目錄結構
    print(f"當前目錄下有: {os.listdir('.')}")
    if os.path.exists("travel-planner"):
        print(f"travel-planner 下有: {os.listdir('travel-planner')}")
